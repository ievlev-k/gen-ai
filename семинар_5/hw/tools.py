...
_METRIC_MAP = {
    ...
    "fx_USD": lambda on_date: get_fx_rate("USD", on_date),
    "fx_EUR": lambda on_date: get_fx_rate("EUR", on_date),
    "fx_CNY": lambda on_date: get_fx_rate("CNY", on_date),
    "cpi": lambda year, month: get_inflation(year, month),
    "unemployment": lambda year, month: get_unemployment(year, month),
}


def _resample_date(period: str) -> tuple:
    period = period.strip()
    if len(period) == 7:
        # YYYY-MM -> YYYY-MM-DD
        return period + "-01"
...
def _extract_value_and_date(obs: dict, metric: str) -> tuple:
    if "error" in obs:
        raise ValueError(f"инструмент вернул ошибку: {obs['error']}")
    if metric == "key_rate":
        return obs["date"], obs["rate"]
    if metric.startswith("fx_"):
        return obs["date"], obs["rate"]
    if metric in ("cpi", "unemployment"):
        key = "cpi_yoy" if metric == "cpi" else "unemployment"
        date_str = f"{obs['year']}-{obs['month']:02d}"
        return date_str, obs[key]
...
def compare_periods(
    ...
    if fn is None:
        return {"error": f"неизвестная метрика: {metric}. Доступные: {list(_METRIC_MAP.keys())}"}

    date_a = _resample_date(period_a)
    date_b = _resample_date(period_b)

    if metric in ("cpi", "unemployment"):
        ya, mo_a = int(date_a[:4]), int(date_a[5:7])
        yb, mo_b = int(date_b[:4]), int(date_b[5:7])
        obs_a = fn(ya, mo_a)
        obs_b = fn(yb, mo_b)
    else:
        # fx_*, key_rate: на_date
        obs_a = fn(on_date=date_a)
        obs_b = fn(on_date=date_b)

    da, va = _extract_value_and_date(obs_a, metric)
    db, vb = _extract_value_and_date(obs_b, metric)

    delta = round(vb - va, 6)
    ratio = round(vb / va, 6) if va != 0 else None
